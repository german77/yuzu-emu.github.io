...
+++
This tool allows you to enter perfectly timed, precise controller inputs to a game with what would be a near-impossible degree of accuracy for a human.
You set and lock the different analog sticks to certain positions, as well as specify what buttons are to be pressed or released.

# How to use

To play back TAS scripts on Yuzu, first select the folder with scripts in the configuration menu below
...
If you decide to run the script over and over again `Loop script` will do just that. The tool would not stop until you specifically send the stop hotkey `CTRL+F5`.
